(function ($, UNDEF) {
    $.fn.noUiSlider = function (options) {
        var namespace = '.nui', all = $(document), actions = {
        ...
        }, $VAL = $.fn.val, clsList = ['noUi-base', 'noUi-origin', 'noUi-handle', 'noUi-input', 'noUi-active', 'noUi-state-tap', 'noUi-target', '-lower', '-upper', 'noUi-connect', 'noUi-vertical', 'noUi-horizontal', 'handles', 'noUi-background', 'noUi-z-index'], stdCls = {
            base: [clsList[0]],
            origin: [clsList[1]],
            handle: [clsList[2]]
        }, percentage = {
        ...
        function fixEvent(e, preventDefault) {
            ...
            var jQueryEvent = e, touch = e.type.indexOf('touch') === 0, mouse = e.type.indexOf('mouse') === 0, pointer = e.type.indexOf('MSPointer') === 0, x, y;
            e = e.originalEvent;
            if (touch) {
        ...
        function test(o, set) {
            ...
            var TESTS = {
                "handles": {
                    r: true, t: function (o, q) {
                        q = parseInt(q, 10);
                        return (q === 1 || q === 2);
                    }
                }, "range": {
                    r: true, t: function (o, q, w) {
                        if (q.length !== 2) {
                            return false;
                        }
        ...
        function setHandle(handle, to, forgive) {
            var nui = handle.data('nui').options, handles = handle.data('nui').base.data(clsList[12]), style = handle.data('nui').style, hLimit;
            if (!$.isNumeric(to)) {
                return false;
            }
        ...
        function store(handle, S) {
            ...
            if (S.to[i]instanceof $) {
                return S.to[i].data({
                    ...
                    var arr = [null, null];
                    arr[i] = $(this).val();
                    $(this).data('target').val(arr, {trusted: false});
                }).on('change' + namespace, function () {
                    call($(this).data('handle').data('nui').options.set, $(this).data('target'));
                });
            }
            if (typeof S.to[i] === "string") {
                return $('<input type="hidden" class="' + clsList[3] + '" name="' + S.to[i] + '">').appendTo(handle).change(stopPropagation);
            }
            if (S.to[i] === false) {
                return {
                    val: function (a) {
        ...
        function move(event) {
            ...
            var base = event.pass.base, style = base.data('style'), proposal = event.x - event.pass.startEvent.x, baseSize = style === 'left' ? base.width() : base.height();
            if (style === 'top') {
        ...
        function end(event) {
            ...
            var base = event.data.base, handle = event.data.handle;
        ...
        function start(event) {
            ...
            var handle = event.pass.handle, position = handle[0].gPct(handle.data('nui').style);
            handle.children().addClass(clsList[4]);
            all.on(actions.move, {startEvent: event, position: position, base: event.pass.base, handle: handle}, move);
        ...
        function tap(event) {
            ...
            var i, handle, hCenter, base = event.pass.base, handles = event.pass.handles, style = base.data('style'), eventXY = event[style === 'left' ? 'x' : 'y'], baseSize = style === 'left' ? base.width() : base.height(), offset = {
                handles: [],
                base: {left: base.offset().left, top: base.offset().top}
            };
            for (i = 0; i < handles.length; i++) {
                offset.handles.push({left: handles[i].offset().left, top: handles[i].offset().top});
            }
        ...
        function create() {
            return this.each(function (index, target) {
                ...
                var i, style, decimals, handle, base = $('<div/>').appendTo(target), handles = [], cls = {
                    base: stdCls.base,
                ...
                options = $.extend({handles: 2, margin: 0, orientation: "horizontal"}, options) || {};
                if (!options.serialization) {
                    options.serialization = {to: [false, false], resolution: 0.01, mark: '.'};
                }
                test(options, target);
                ...
                target.data({base: base, mark: options.S.mark, decimals: decimals});
                for (i = 0; i < options.handles; i++) {
                    ...
                    handle.children().on(actions.start, {
                        base: base,
                        handle: handle
                    }, start).on(actions.end, {base: base, handle: handle}, selfEnd);
                    handle.data('nui', {
                        target: target,
                        decimals: decimals,
                        options: options,
                ...
                base.data({options: options, handles: handles, style: style});
                target.data({handles: handles});
        ...
        function val(args, modifiers) {
            ...
            return this.each(function (i, target) {
                ...
                $.each($(this).data(clsList[12]), function (j, handle) {
                    ...
                    var value, current, range = handle.data('nui').options.range, to = args[j], result;
